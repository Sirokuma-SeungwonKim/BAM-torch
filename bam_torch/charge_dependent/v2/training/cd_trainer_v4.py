"""
CDTrainerV4 (BAM-torch CD Ver.2, 2026-05-21~).

Derived from CDTrainerV3 (Ver.1). Only `set_model` overridden to instantiate
ChargeRACEv4 (Ver.2 model with U_SLR).

All other behavior (loss, scheduler, save, EMA) inherits unchanged.
"""
...
class CDTrainerV4(CDTrainerV3):
    """Phase 4 (Ver.2 CENT2 FULL) trainer — ChargeRACEv4."""
    ...
    def set_model(self):
        ...
        # Ver.1 charge-dependent parameters
        cd_kwargs = parse_charge_config(self.json_data)
        model_kwargs.update(cd_kwargs)

        # Ver.2 specific: U_SLR parameters from charge config
        charge_cfg = self.json_data.get('charge', {})
        model_kwargs['slr_kernel'] = charge_cfg.get('slr_kernel', 'cent2')
        model_kwargs['slr_lambda'] = charge_cfg.get('slr_lambda', 0.340)  # 0.18 a_0^-1, CENT2 paper optimal
        ...
        # Direct instantiation (Ver.2 model not in legacy registry)
        self.model = ChargeRACEv4(**model_kwargs)
        ...
        self.msg += (
            f"\n\033[33m -- Ver.2 (Phase 4): CENT2 FULL\033[0m\n"
